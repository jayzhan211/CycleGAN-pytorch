...
from .networks_unet import Encoder, Generator
import torch
...
class UNETModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--mapping-fmaps', type=int, default=2526, help='input channel')
    ...
    def __init__(self, opt):
        ...
        mapping_layers = opt.mapping_layers
        mapping_fmaps = opt. mapping_fmaps
        ...
        ###################################
        if self.isTrain:
            self.disA = Encoder(latent_size=latent_size, mapping_layers=mapping_layers, mapping_fmaps= mapping_fmaps).to(self.device)
            self.disB = Encoder(latent_size=latent_size, mapping_layers=mapping_layers, mapping_fmaps= mapping_fmaps).to(self.device)
        self.genA2B = Generator(latent_size=latent_size).to(self.device)
        self.genB2A = Generator(latent_size=latent_size).to(self.device)
