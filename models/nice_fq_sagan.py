...
from models.networks import NICEV2ResnetGenerator, NICESADiscriminator
from models.vq_layer import VectorQuantizerEMA


class NICEFQSAGANModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--quantization_layer', type=str, default='123', help='layer to quantize')
        ...
        parser.set_defaults(netD='nice_dis_sa_fq')
        if is_train:
    ...
    def __init__(self, opt):
        super(NICEFQSAGANModel, self).__init__(opt)
        self.loss_names = ['G', 'D']
        ...
        if opt.netG == 'nice_gen_v2':
            self.genA2B = NICEV2ResnetGenerator(opt.input_nc, opt.output_nc, opt.ngf, opt.n_res, opt.img_size).to(
                self.device)
            self.genB2A = NICEV2ResnetGenerator(opt.output_nc, opt.input_nc, opt.ngf, opt.n_res, opt.img_size).to(
                self.device)
        else:
        ...
        if self.isTrain:
            ...
            self.quant_layers = [int(x) for x in opt.quantization_layer]
            layerwise_channel = [64, 128, 256, 512, 1024, 2028]
            num_embed = [5, 6, 7, 8, 9, 10]

            self.quantize = {}
            for layer in self.quant_layers:
                self.quantize[layer] = VectorQuantizerEMA(dim=layerwise_channel[layer], n_embed=2 ** num_embed[layer], commitment_cost=opt.commitment_cost, decay=self.decay)
